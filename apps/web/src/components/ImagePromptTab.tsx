...
/**
 * ImagePromptTab — UI untuk Image Prompt Engine + Budget Guard (Fase 4).
 *
 * Alur:
 * 1. Pilih Scene → pilih Shot
 * 2. Preview prompt konseptual (compile)
 * 3. Buat Generation Job → muncul estimasi biaya
 * 4. Approve/Reject estimasi (Budget Guard — approval eksplisit)
 * 5. Submit ke adapter (Flux) → hasil gambar
 */
export default function ImagePromptTab({ projectId, scenes }: any) {
  ...
  async function loadCapabilities() {
    try {
      ...
      setCapabilities({ imageGenerationEnabled: true });
    }
  }
  ...
  async function loadJobs(shotId: string) {
    try {
      const data = await api.getImageJobsByShot(projectId, shotId);
      setJobs(data);
    } catch (e: any) {
      // ignore
    }
  ...
  async function handleCompile() {
    ...
    setPreview(null);
    try {
      const result = await api.compileImagePrompt(projectId, selectedShotId);
      setPreview(result);
    } catch (e: any) {
      setError(e.message);
    } finally {
  ...
  async function handleCreateJob() {
    ...
    setError('');
    try {
      await api.createImageJob(projectId, selectedShotId);
  ...
  async function handleApprove(jobId: string) {
    ...
    setError('');
    try {
      await api.approveImageJob(projectId, jobId);
      await loadJobs(selectedShotId);
    } catch (e: any) {
      setError(e.message);
    } finally {
  ...
  async function handleReject(jobId: string) {
    ...
    setError('');
    try {
      await api.rejectImageJob(projectId, jobId);
      await loadJobs(selectedShotId);
    } catch (e: any) {
      setError(e.message);
    } finally {
  ...
  async function handleSubmit(jobId: string) {
    ...
    setError('');
    try {
      await api.submitImageJob(projectId, jobId);
      await loadJobs(selectedShotId);
    } catch (e: any) {
      setError(e.message);
    } finally {
  ...
  const imageGenEnabled = capabilities?.imageGenerationEnabled !== false;

  return (
    <div className="space-y-4">
      <h2 className="text-lg font-semibold">Image Prompt Engine</h2>

      {!imageGenEnabled && (
        <div className="p-3 bg-yellow-50 border border-yellow-200 rounded text-yellow-800 text-sm">
          Image generation tidak aktif untuk project ini. Aktifkan di pengaturan
      ...
      {selectedShotId && imageGenEnabled && (
        <div className="flex gap-2">
          ...
          >
            {loading ? 'Memproses...' : 'Preview Prompt'}
          </button>
          <button
      ...
      {/* Preview Prompt Konseptual */}
      {preview && (
        <div className="border rounded p-4 bg-gray-50">
          <h3 className="font-medium mb-2">Preview Prompt Konseptual</h3>
          ...
          <div className="mt-2 text-xs text-gray-500">
            Bible versions:{' '}
            {JSON.stringify(preview.bibleVersions)}
          </div>
        </div>
      )}

      {/* Daftar Generation Jobs */}
      {selectedShotId && jobs.length > 0 && (
        <div>
          <h3 className="font-medium mb-2">Generation Jobs</h3>
                ...
                {job.outputAssetUrl && (
                  <div className="mb-2">
                    <img
                      ...
                      alt={`Hasil generation job v${job.promptVersion}`}
